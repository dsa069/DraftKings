...
public class ServletImpl extends HttpServlet {
    ...
    private void handleRequest(HttpServletRequest req, HttpServletResponse response) throws IOException {
        boolean json = wantsJson(req);
        response.setContentType(json ? "application/json; charset=UTF-8" : "text/html; charset=UTF-8");
        ...
        if (action == null) {
            if (json) {
                out.println(buildJsonError("Accion no especificada."));
            } else {
        ...
        if ("Enviar".equals(action)) {
            ...
            actionEnviar(out, noticia, json);
        } else if ("Obtener todas".equals(action)) {
            actionObtenerTodas(out, json);
            ...
            try {
                ...
                actionLeerEn(out, indice, json);
            } catch (NumberFormatException e) {
                if (json) {
                    out.println(buildJsonError("Debes indicar un indice numerico valido."));
                } else {
            ...
            if (json) {
                out.println(buildJsonError("Accion '" + action + "' no reconocida."));
            } else {
    ...
    protected void actionEnviar(PrintWriter out, Noticia noticia, boolean json) throws IOException {
        try {
            getreference();
            List<String> errores = NoticiaValidator.validate(noticia);
            if (!errores.isEmpty()) {
                writeError(out, json, joinErrors(errores));
                return;
            }
            ...
            if (!validateAgainstConfiguredXsd(noticiaXML)) {
                writeError(out, json, "La noticia no cumple el esquema XML de noticias.");
                return;
            }
            ...
            writeSuccess(out, json, "Enviar", "Noticia insertada correctamente.");
        } catch (Exception e) {
            writeError(out, json, getErrorMessage(e));
        }
    }

    protected void actionObtenerTodas(PrintWriter out, boolean json) {
        try {
            getreference();
            String[] noticias = bufferImpl.obtener_todas();
            if (noticias == null || noticias.length == 0) {
                writeError(out, json, "El buffer esta vacio.");
                return;
            }

            if (json) {
                out.println(buildJsonNoticias(noticias));
                return;
            }
    ...
    protected void actionLeerEn(PrintWriter out, int indice, boolean json) {
        try {
            getreference();
            StringHolder aux = new StringHolder();
            if (!bufferImpl.read_en(indice, aux)) {
                writeError(out, json, "Indice invalido o buffer vacio.");
                return;
            }
            ...
            if (json) {
                out.println(buildJsonNoticia(indice, noticiasLeidas.get(0)));
                return;
            }
    ...
    private void getreference()
        ...
        try {
            ...
            ncRef = retryWithFallbackHost(orbHost, orbPort, props);
        } catch (Exception e) {
    ...
    private void writeSuccess(PrintWriter out, boolean json, String action, String message) {
        if (json) {
            out.println("{\"ok\":true,\"action\":\"" + escapeJson(action) + "\",\"message\":\""
                    + escapeJson(message) + "\"}");
            return;
        }
        printResultado(out, "<font color='#2EFE64'>" + message + "</font>");
    }

    private void writeError(PrintWriter out, boolean json, String message) {
        if (json) {
            out.println(buildJsonError(message));
            return;
        }
        printResultado(out, "<font color='#DF0101'>" + message + "</font>");
    }

    private String buildJsonNoticias(String[] noticias) {
        StringBuilder json = new StringBuilder();
        json.append("{\"ok\":true,\"action\":\"Obtener todas\",\"count\":").append(noticias.length)
                .append(",\"noticias\":[");
        for (int i = 0; i < noticias.length; i++) {
            if (i > 0) {
                json.append(',');
            }
            int separador = noticias[i].indexOf('|');
            int indice = i;
            String noticiaXML = noticias[i];
            if (separador >= 0) {
                try {
                    indice = Integer.parseInt(noticias[i].substring(0, separador));
                } catch (NumberFormatException e) {
                    indice = i;
                }
                noticiaXML = noticias[i].substring(separador + 1);
            }
            List<Noticia> noticiasLeidas = XMLDecoder.decodeXML(noticiaXML, 1);
            if (noticiasLeidas.isEmpty()) {
                json.append("{\"indice\":").append(indice)
                        .append(",\"error\":\"No se ha podido decodificar la noticia.\"}");
                continue;
            }
            json.append(buildNoticiaJson(indice, noticiasLeidas.get(0)));
        }
        json.append("]}");
        return json.toString();
    }

    private String buildJsonNoticia(int indice, Noticia noticia) {
        StringBuilder json = new StringBuilder();
        json.append("{\"ok\":true,\"action\":\"Leer en\",\"indice\":").append(indice)
                .append(",\"noticia\":").append(buildNoticiaJson(indice, noticia)).append("}");
        return json.toString();
    }

    private String buildNoticiaJson(int indice, Noticia noticia) {
        StringBuilder json = new StringBuilder();
        json.append("{\"indice\":").append(indice)
                .append(",\"fecha\":\"").append(escapeJson(noticia.getFecha())).append("\"")
                .append(",\"jugador\":\"").append(escapeJson(noticia.getJugador())).append("\"")
                .append(",\"interes\":\"").append(escapeJson(noticia.getInteresValue())).append("\"")
                .append(",\"titulo\":\"").append(escapeJson(noticia.getTitulo())).append("\"")
                .append(",\"descripcion\":\"").append(escapeJson(noticia.getDescripcion())).append("\"")
                .append(",\"etiquetas\":[");
        ...
        for (int i = 0; i < etiquetas.size(); i++) {
            ...
            json.append('"').append(escapeJson(etiquetas.get(i))).append('"');
        }
        json.append("]}");
        return json.toString();
    }
    ...
    private String formatNoticia(Noticia noticia) {
        return "fecha=" + noticia.getFecha() + ", jugador=" + noticia.getJugador() + ", interes="
                + noticia.getInteresValue() + ", titulo=" + noticia.getTitulo() + ", descripcion="
    ...
    private NamingContextExt retryWithFallbackHost(String orbHost, String orbPort, java.util.Properties props)
            throws Exception {
